/**
 * AMA11Y Extension — Side Panel Logic
...
(function () {
  ...
  /* ================================================================
     ON LOAD — recover results if panel opened after audit completed
  ================================================================ */

  document.addEventListener('DOMContentLoaded', () => {
    chrome.storage.session.get('lastAudit', (data) => {
      if (data && data.lastAudit && data.lastAudit.type === 'audit-results') {
        ...
        onAuditComplete();
        // Clear so stale results don't reappear on next open
  ...
  function onAuditComplete() {
    ...
    announce(`AMA11Y audit complete. ${fail} failures, ${warn} warnings, ${pass} passes found. ${allFindings.length} total findings across 13 engines.`);
  }
  ...
  exportJson.addEventListener('click', () => {
    ...
    downloadFile(JSON.stringify(data, null, 2), 'ama11y-audit.json', 'application/json');
    announce('JSON exported.');
  });

  exportHtml.addEventListener('click', () => {
    const html = generateHtmlReport();
    downloadFile(html, 'ama11y-report.html', 'text/html');
  ...
  exportCsv.addEventListener('click', () => {
    ...
    downloadFile(csv, 'ama11y-audit.csv', 'text/csv');
  ...
  exportText.addEventListener('click', () => {
    ...
    lines.push('Generated by AMA11Y v2.0.0 — Private and Confidential');
...
<title>AMA11Y Audit Report — ${escHtml(auditMeta.pageTitle)}</title>
<style>
...
<header role="banner">
  ...
  <p>Page: ${escHtml(auditMeta.pageTitle)} | URL: ${escHtml(auditMeta.pageUrl)} | Date: ${escHtml(auditMeta.timestamp)} | Tool: AMA11Y v2.0.0</p>
</header>
...
<footer role="contentinfo">
  <p>Generated by AMA11Y v2.0.0 — ama11y.akhileshmalani.com — Private and Confidential — Akhilesh Malani</p>
</footer>
</body>
</html>`;
  }
